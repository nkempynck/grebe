...
import { SyncPanel } from "./StatsPanel";
 ...
 *  game's all-time standing lives in that game's badges panel, under Stats. */
...
/** Identity and settings only: who you are, how you appear on the boards, and
 *  cross-device sync. Stats and badges live in their own section (see StatsTabs) —
 *  they were the same endless scroll, and they answer a different question. */
export function AccountPanel({ player }: Props) {
  return (
    <>
      {player.session && <Profile player={player} />}
      <SyncPanel player={player} />
    </>
  );
}
